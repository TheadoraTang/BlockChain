...
public class MinerNode extends Thread {
    ...
    public BlockBody getBlockBody(Transaction[] transactions) {
        ...
        // 计算交易的Merkle树根哈希值
        StringBuilder merkleStringBuilder = new StringBuilder();
        for (Transaction transaction : transactions) {
            merkleStringBuilder.append(SHA256Util.sha256Digest(transaction.toString()));
        }
        String merkleRootHash = SHA256Util.sha256Digest(merkleStringBuilder.toString());
    ...
    private void mine(BlockBody blockBody) {
        ...
        while (true) {
            ...
            if (blockHash.startsWith(MinerUtil.hashPrefixTarget())) {
                ...
                System.out.println("And the hash of this Block is : " + blockHash +
    ...
    public Block getBlock(BlockBody blockBody) {
        ...
        String previousBlockHash = lastBlock != null ? SHA256Util.sha256Digest(lastBlock.toString()) : "0";
        BlockHeader blockHeader = new BlockHeader(previousBlockHash, blockBody.getMerkleRootHash(), System.currentTimeMillis());
